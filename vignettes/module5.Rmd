...
```{r}
#Transformation d'une série en logarithme
log_ts <- function(serie_ts,log=FALSE){
  if(log==TRUE){
    x <- log(eval(parse(text = serie_ts)))
  }else{
    x <- eval(parse(text = serie_ts))
    }
  x
}

log_airpass <- log_ts(serie_ts = "airpass",log=TRUE)
airpass <- log_ts(serie_ts = "airpass",log = FALSE)
...
log_airpass_cvs <- exp(cvs_reg("log_airpass"))

dygraph(airpass_cvs)
dygraph(log_airpass_cvs)
```
