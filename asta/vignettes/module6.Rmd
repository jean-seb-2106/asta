...
wflow_lr1 <- 
  workflow() %>% 
  add_model(mod_lr) %>% #ajout de la regression logistique
  add_recipe(rec1) #on garde toutes les variables

#pour la logistique et la deuxième recette
wflow_lr2 <- 
  workflow() %>% 
  add_model(mod_lr) %>% #ajout de la regression logistique
  add_recipe(rec2) #avec deux variables en moins

#Pour la random forest avec la première recette
wflow_rf1 <- 
  workflow() %>% 
  add_model(mod_rf) %>% 
  add_recipe(rec1)

#Pour la random forest avec la troisième recette
wflow_rf2 <- 
  workflow() %>% 
  add_model(mod_rf) %>% 
  add_recipe(rec3)
...
set.seed(345)
nb_folds <- 10
folds <- vfold_cv(train_data, v = nb_folds)#paramétrage de la validation croisée 
...
#On peut créer une fonction qui donne en sortie le modèle fité
...
fit_lr1 %>% fit_resamples(folds) %>% collect_metrics()
fit_lr2 %>% fit_resamples(folds) %>% collect_metrics()
fit_rf1 %>% fit_resamples(folds) %>% collect_metrics()
fit_rf2 %>% fit_resamples(folds) %>% collect_metrics()

```


```{r}
#renvoie les tables avec les prédictions sur le fichier d'entraînement
pred_training_lr1 <- augment(fit_lr1, train_data)
pred_training_lr2 <- augment(fit_rf1, train_data) 
pred_training_rf1
pred_training_rf2

DT::datatable(lr1_training_pred)
DT::datatable(rf1_training_pred)
