...
#                                      prop = part_training)
data_split <- initial_validation_split(vins,strata = .data[[var_strata]])
...
rec <- rec3 #paramètre à changer pour changer de recette
rm(rec1,rec2,rec3)#pour nettoyer l'environnement
```
...
mod <- mod_knn #paramètre à changer pour changer de modèle

rm(mod_lr,mod_ridge,mod_lasso,mod_rf,mod_tree,mod_knn)
```
...
```{r}
#Entraînement du modèle (non fité)
fit <- 
  wflow %>% 
  fit(data = train_data) 
...
# fit_rs <- fit %>% fit_resamples(folds) 
...
pred_training <- augment(fit, train_data)
pred_training

# DT::datatable(pred_training)
...
pred_testing <- augment(fit, test_data) #renvoie une base avec aussi les probas 
