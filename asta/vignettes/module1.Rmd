...
```{r}
#Autre fonction pour créer un nuage de points quali-quanti avec moyennes

#paramètres
df <- grandile
varquali <- "DIPL"
varquanti <- "REV_DISPONIBLE"

varquali_lib <- paste0("LIB_",varquali)
moy <- mean(df[,varquanti])
ggplot(df,aes(x = .data[[varquali]],y=.data[[varquanti]],color=.data[[varquali_lib]])) +
